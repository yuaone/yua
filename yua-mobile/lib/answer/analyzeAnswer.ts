export type AnswerBodyType = "paragraph" | "markdown" | "code";
...
export function analyzeAnswer(content: string): AnswerAnalysis {
  ...
  const hasMath = REGEX.mathInline.test(trimmed) || REGEX.mathBlock.test(trimmed);
  const sentenceCount = trimmed.match(REGEX.sentence)?.length ?? 0;
  const paragraphCount = paragraphs.length;

  let bodyType: AnswerBodyType = "paragraph";
  if (hasFencedCode && !hasList && paragraphCount <= 2) {
    bodyType = "code";
  } else if (hasFencedCode || hasList || hasHeader || hasBlockquote || hasMath) {
    bodyType = "markdown";
  }

  const isShortAnswer = charLength < 120 && paragraphCount <= 1;
  const isExplanationLike = paragraphCount >= 2 && sentenceCount >= 3;

  const isExpandable =
    isShortAnswer || (!hasCode && sentenceCount < 6) || (paragraphCount === 1 && charLength < 300);
