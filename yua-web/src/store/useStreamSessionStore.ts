...
export type ThinkingSummaryItem = {
  id: string;        // narration.chunkId와 동일
  tick: number;      // 생성 순서 (monotonic)
  title: string | null;
  summary: string | null;
...
export type OverlayChunk = {
  chunkId: string;
  index: number;
  groupIndex?: number; // 🔥 segment grouping id (segmentIndex 기반)
  at: number;
  ...
  metaTool?: string | null;
   meta?: any; // 🔥 raw activity meta 보존 (query 등 포함)
  sources?: {
    id: string;
    label: string;
    url: string;
...
function mergeById<T extends { chunkId: string }>(prev: T[], next: T[]) {
  ...
  if (next.length === 0) return prev; // 🔥 empty array 덮어쓰기 방지 (청크 사라짐 버그 방어)
...
// 🔥 Activity PATCH Buffer (ChatGPT 스타일 리듬)
...
type StreamSession = {
 ...
 thinking: {
  ...
  primarySummaryId?: string | null; // 🔥 대표 요약 (Drawer title)
  drawerUserClosed?: boolean; // 🔥 사용자 의도 (영속)
...
firstAnswerTokenAt: number | null;
  ...
  now: number | null;
  activeChunkId: string | null; // 🔥 SSOT
  imageAnalyzing: boolean; // 🔥 사진+텍스트 분석용 오버레이 플래그
  debugDeepMessageId?: string | null; // 🔥 디버깅용: DEEP 발행 메시지 고정
};
...
if (state.session?.finalized === true) {
  ...
  // 🔥 SSOT: thinking signal 증가 (의미 이벤트만)
  // 🔥 SSOT: DEEP에서는 "첫 activity"가 곧 thinking 시작 신호다.
  // - stage 의존 제거
  // - reasoning_summary 없어도 seed activity로 시작 가능
  ...
  // 🔥 Activity는 profile과 무관하게 항상 streaming 상태 유지
  ...
  // ✅ SSOT: summary = chunks에서 파생되는 "진짜 JSON 배열"
  // - BEGIN: ACTIVE seed
  // - END(OK/FAILED): DONE seed
  ...
  // ✅ SSOT: tool chips → tools
  // 🔥 TOOL을 chunk에 귀속 (timeline SSOT)
  const firstChip =
  ...
  // 🔥 NEW: meta.sources 우선 → chips fallback (ChatGPT 스타일)
...
if (raw === "1") {
  ...
  hydrateFromSnapshot: (snapshot: any) =>
    set((s) => {
      ...
      const chunks = raw.map((c: any, i: number) => ({
        // 🔒 SSOT: meta.sources -> chunk.sources 복원
  ...
  set((s) => {
    // 🔥 이미 동일 messageId 세션이 있으면 start 차단
  ...
  update: (patch) =>
    set((s) => {
      const nextSession = {
        ...s.session,
        ...patch,
        // 🔒 nested object merge (생각/인라인은 부분 patch가 많아서 안전하게)
        thinking: (patch as any).thinking
          ? { ...s.session.thinking, ...(patch as any).thinking }
          : s.session.thinking,

        // 🔥 배열 필드 merge: stream 중 chunks/summaries/tools가 "빈 배열로 덮어쓰기" 되는 버그 방지
chunks: Array.isArray((patch as any).chunks)
  ...
  : s.session.chunks,
   ...
   /**
    +   * ✅ SSOT: Activity → OverlayChunk 단일 타임라인
   * - Panel(활동): chunks(source=ACTIVITY)
   * - Inline: 동일 chunkId의 inline/body를 같은 tick에 갱신
   * - Panel title: session.label을 active chunk.title로 동기화
   */
 applyActivity: (payload) => {
  ...
  /** 🔥 DEEP: reasoning_block → OverlayChunk */
...
thinking: shouldHoldThinking
  ...
  : {
      ...s.session.thinking,
      // 🔥 FINAL 이후에도 thinking.active는 유지
      // panel은 session.active + chunks 기준으로 유지
      active: false,
      phase: "answer",
    },
minSkeletonUntil:
