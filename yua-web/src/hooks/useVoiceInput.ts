...
export function useVoiceInput({
  ...
  // Audio level animation loop (throttled to ~15fps to reduce re-renders)
  const lastLevelUpdateRef = useRef(0);
  const updateAudioLevel = useCallback(() => {
    const analyser = analyserRef.current;
    if (!analyser) return;
    const now = performance.now();
    if (now - lastLevelUpdateRef.current > 66) { // ~15fps
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      // AudioContext for level metering (resume for Safari)
  ...
  const cancelRecording = useCallback(() => {
    ...
    // cleanup is called by recorder.onstop handler; if recorder isn't active, cleanup directly
    if (mediaRecorderRef.current?.state === "recording") {
      mediaRecorderRef.current.stop(); // triggers onstop → cleanup
    } else {
      cleanup();
      setIsRecording(false);
    }
