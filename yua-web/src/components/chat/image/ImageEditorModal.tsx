...
// iOS Safari limits canvas to ~16M pixels; cap to safe size
...
export default function ImageEditorModal({
  ...
  const computeDisplay = useCallback(() => {
    ...
    // Reserve space for toolbar (bottom 80px on mobile, top toolbar on desktop)
  ...
  const handleImgLoad = useCallback(() => {
    ...
    // Cap to MAX_CANVAS_DIM to prevent iOS canvas crash (H-05)
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black/90">
      {/* ===== Top bar (desktop) ===== */}
      <div className="hidden md:flex items-center justify-between px-4 py-3 bg-black/60">
      ...
      <div className="md:hidden flex flex-col gap-2 px-3 pb-[env(safe-area-inset-bottom,12px)] pt-2 bg-black/70">
        {/* Color + width row */}
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          ...
          <div className="shrink-0 w-px h-6 bg-white/20 mx-1" />
          ...
          {LINE_WIDTHS.map((w) => (
            <button
              ...
              `}
            >
              <Minus size={16} className="text-white" strokeWidth={w} />
            </button>
          ))}
          ...
          <button
            onClick={handleUndo}
            disabled={strokes.length === 0}
            className="shrink-0 p-2 rounded-lg text-white disabled:opacity-30"
          >
            <Undo2 size={20} />
          </button>
          <button
        ...
        <div className="flex gap-2">
          <button
            onClick={onCancel}
            className="flex-1 h-12 rounded-xl bg-white/10 text-white text-sm font-medium active:bg-white/20 transition"
          >
            취소
          </button>
          <button
            onClick={handleConfirm}
            className="flex-1 h-12 rounded-xl bg-white text-black text-sm font-medium active:bg-white/90 transition"
          >
            <Check size={18} className="inline mr-1 -mt-0.5" />
            적용
          </button>
        </div>
      </div>
