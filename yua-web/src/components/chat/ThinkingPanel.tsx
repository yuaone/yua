...
drawerOpenReason?: "AUTO" | "USER_COMPLETE";
  ...
  export default function ThinkingPanel({
    ...
    // ✅ SSOT: 새로고침 복원은 snapshot이 아니라 message.meta(thinkingProfile) 기반이어야 함.
    // ChatMain이 session.thinkingProfile을 세팅해주므로 여기서도 신뢰한다.
 ...
 // 🔥 NORMAL 모드에서도 SEARCH query 인라인 허용
...
const skeletonVisible =
 ...
 const title = null; // 🔥 인라인에서는 title 표시 안함
    const activityLabel =
 ...
 const displayTitle =
   finalized
     ? "활동 완료"
     : activityLabel; // 🔥 reasoning title 사용 안함
...
// 🔥 title swap trigger (key remount 기반)
...
useEffect(() => {
 ...
 const openDrawer = () => {
  // ✅ StreamOverlay가 준 onOpen이 있으면 그걸 신뢰 (History-safe)
  if (onOpen) {
    onOpen();
    return;
  }
