...
export default function ChatPlusMenu({
  ...
  useEffect(() => {
    ...
    // capture 단계에서 잡아야 안정적(버튼 클릭/포커스보다 먼저 처리)
  ...
  if (view === "profile") {
    return (
      ...
      >
        ...
        <div className="px-2 py-1 text-[12px] text-gray-400 dark:text-[var(--text-muted)]">Thinking Profile</div>
        ...
        {items.map((it) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <div className="text-sm text-gray-900 dark:text-[var(--text-primary)]">{it.label}</div>
                <div className="text-[12px] text-gray-500 dark:text-[var(--text-secondary)]">{it.desc}</div>
              </div>
            </button>
          );
        })}
